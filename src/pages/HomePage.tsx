  ...
  return (
    <div className="min-h-screen bg-surface-bg">
      <section className="bg-surface-panel border-b border-border-input">
        <div className="max-w-[1400px] mx-auto px-10 py-10 flex flex-col gap-6">
          ...
          {(isOnboardingLoading || onboardingProblems.length > 0) && (
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold text-text-primary">처음 풀어보세요</h2>
                <Link to="/problems?maxDifficulty=3" className="flex items-center gap-1 text-[13px] text-text-muted hover:text-text-secondary transition-colors">
                  더보기
              ...
              <div className="flex gap-3 overflow-x-auto pb-1">
                {isOnboardingLoading ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <div key={i} className="shrink-0 w-52 h-24 rounded-lg bg-surface-muted animate-pulse" />
                  ))
                ) : (
                  onboardingProblems.map((p) => (
                    <Link
                      key={p.id}
                      to={`/problems/${p.id}`}
                      className="shrink-0 w-52 p-4 rounded-lg bg-surface-bg border border-border-input hover:border-brand-primary hover:bg-surface-muted transition-all flex flex-col gap-2"
                    >
                      ...
                      <span className="text-xs text-text-muted mt-auto">
                        정답률 {p.acceptanceRate != null ? `${p.acceptanceRate.toFixed(1)}%` : '-'}
                      </span>
                    </Link>
                  ))
                )}
